...
async function readRealSnmp(): Promise<SnmpConfig> {
  ...
  const active = await run("systemctl is-active snmpd");
  snmp.enabled = active.stdout.trim() === "active";
  const conf = await run(`cat ${SNMP_CONF}`);
  if (conf.code === 0) {
  ...
  switch (a.kind) {
    case "ups.setPolicy": {
      ...
      try {
        ...
        await run("systemctl reload-or-restart nut-monitor");
        return ok();
      } catch {
    ...
    case "snmp.update": {
      ...
      try {
        ...
        const toggle = next.enabled
          ? "systemctl enable --now snmpd"
          : "systemctl disable --now snmpd";
        const { code, stderr } = await run(toggle, { timeoutMs: 15000 });
    ...
    case "ups.test": {
      ...
      const { code, stderr } = await run(`upscmd ${upsName} test.battery.start.quick`);
