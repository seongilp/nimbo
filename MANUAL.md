...
배포판을 자동 감지해 필요한 의존성을 설치하고, **프리빌트 번들을 받아**(서버에서
빌드하지 않아 빠름) systemd 서비스로 등록합니다.
...
위 한 줄이면 Caddy 리버스 프록시가 443에서 TLS를 종료합니다. 도메인을 주지 않으면
서버의 기본 IP를 자동 감지해 `https://<서버-IP>` 를 자체 서명 인증서(tls internal)로
띄웁니다. 도메인을 쓰거나 Caddy를 건너뛰려면:

```bash
# 공개 도메인이 있을 때 (진짜 Let's Encrypt 인증서)
curl -fsSL .../bootstrap.sh | sudo bash -s -- --caddy nas.example.com

# 특정 IP로 지정 (Caddy 자체 서명 인증서 = tls internal)
curl -fsSL .../bootstrap.sh | sudo bash -s -- --caddy 192.168.0.10

# 이미 리버스 프록시가 있어 Caddy를 건너뛸 때 (앱은 http://<서버-IP>:PORT)
curl -fsSL .../bootstrap.sh | sudo bash -s -- --no-caddy
```

> 443이 이미 쓰이면 자동으로 10443을 쓰고, 그것도 막혀 있으면 사용할 HTTPS 포트를 물어봅니다.

### 설치가 해주는 일

- Node.js 20(≥20.9) · git · `python3`(+시스템 `libcrypt` — OS 비밀번호 검증에 필요) · (필요 시) 빌드 도구 설치
- 프리빌트 번들을 `/opt/nimbo` 에 설치(릴리스를 못 받으면 소스 빌드로 폴백), `nimbo` 전용 서비스 계정 생성
- `nimbo.service` (앱, 127.0.0.1:3000) · `nimbo-terminal.service` (터미널 PTY, 127.0.0.1:3001) 등록
- 기본적으로 Caddy(HTTPS/443) 설정 + 방화벽 포트 개방 (`--no-caddy` 시 건너뜀)
...
| 설치 방식 | 접속 주소 |
| --- | --- |
| 기본 (자동) | `https://<서버-IP>` (443, 자동 감지 IP · 자체 서명 인증서) |
| `--caddy 도메인` | `https://나스도메인` (443, 진짜 Let's Encrypt 인증서) |
| `--caddy IP` | `https://<서버-IP>` (443, 자체 서명 인증서) |
| `--no-caddy` | `http://<서버-IP>:3000` (자체 프록시를 직접 라우팅) |
...
1. **기존 프록시를 그대로 쓰기** — `--no-caddy` 로 설치하고, 기존 프록시에서 아래처럼
...
2. **다른 포트로 Caddy 실행** — 443이 이미 쓰이면 설치 스크립트가 자동으로 `10443`을
   쓰고, 그것도 막혀 있으면 사용할 HTTPS 포트를 물어봅니다. 이때 접속 주소는
   `https://호스트:10443` 처럼 됩니다.
...
> ⚠️ **첫 로그인이 특별합니다 (꼭 읽으세요)**
> - **처음 로그인한 계정이 관리자(admin)** 가 됩니다(root든 일반 계정이든). 관리자는
>   시스템 변경(서비스 제어, 디스크 교체, 터미널 등)을 할 수 있습니다.
> - 그때 접속한 **네트워크 대역(/24)이 접속 허용 IP로 자동 등록**되고, 이후 로그인은
>   그 대역에서만 됩니다. → **실제로 사용할 네트워크에서 첫 로그인을 하세요.**
> - 이후엔 관리자와, 관리자가 추가한 계정(또는 허용 그룹의 계정)만 로그인할 수 있습니다.
> - 허용 IP·계정은 로그인 후 **Users → Nimbo 접근**에서 바꿉니다(VPN·Tailscale IP도 추가).
> - IP를 잘못 설정해 잠기면: `/etc/nimbo/users.json`의 `allowedCidrs`를 비우고
>   `sudo systemctl restart nimbo`로 복구합니다.
...
| **Storage Manager** | 디스크·파티션·사용량(ZFS 풀 포함 총용량), SMART 건강 상태·온도, ZFS/RAID/LVM 풀 소속 표시 · **마운트**(btrfs·NTFS·exFAT·FAT32 + SMB/NFS 원격) · **파티셔닝**(GPT/MBR 테이블, 생성·삭제·포맷) |
| **권한 (ACL)** | 폴더별 POSIX ACL — 사용자·그룹 단위 rwx, 기본(default) ACL 상속, 하위 재귀 적용 |
...
### 외장 디스크 · NTFS/btrfs 볼륨 마운트하기
`Storage Manager` → **마운트** 탭 → *마운트되지 않은 볼륨*에서 디스크를 고르고 **마운트**.
파일시스템은 자동 감지되며, 마운트 지점은 `/mnt`·`/media`·`/srv` 하위만 허용됩니다
(`NIMBO_MOUNT_ROOTS`로 변경 가능). **부팅 시 자동 마운트**를 켜면 `/etc/fstab`에
UUID 기준으로 기록되고, Nimbo가 쓴 줄만 나중에 UI에서 지울 수 있습니다.

> NTFS는 커널 `ntfs3` 드라이버를 우선 쓰고 없으면 `ntfs-3g`로 넘어갑니다.
> exFAT은 `exfatprogs`, SMB는 `cifs-utils`, NFS는 `nfs-common` 패키지가 필요하며,
> 없으면 해당 항목이 "미설치"로 표시됩니다.

### Synology 등 다른 NAS 공유 폴더 연결하기
`Storage Manager` → **마운트** 탭 → **네트워크 연결**. SMB/CIFS면 공유 이름(`photo`),
NFS면 내보내기 경로(`/volume1/photo`)를 입력합니다. SMB 비밀번호는 `/etc/fstab`이
아니라 권한 `0600`의 자격 증명 파일에 저장되므로, 자동 마운트를 켜도 비밀번호가
평문으로 노출되지 않습니다.

### 디스크 파티셔닝 · 포맷하기
`Storage Manager` → **파티션** 탭. 파티션 테이블 생성(GPT/MBR), 파티션 생성·삭제·포맷을
할 수 있습니다. **파괴적 작업이라 안전장치가 있습니다.**

- 마운트된 디스크, ZFS/RAID/LVM/LUKS 구성원, 스왑, 루트(시스템) 디스크는 **잠금** 표시되며 변경이 거부됩니다.
- 모든 작업은 장치 경로(`/dev/sdb`)를 그대로 다시 입력해야 실행됩니다.
- 파티션 생성 시 파일시스템을 같이 고르면 생성 직후 새 파티션만 골라 포맷합니다.

### 공유 폴더에 사용자별 세부 권한(ACL) 주기
`권한 (ACL)` 앱 → 상단에서 공유 폴더 버튼을 누르거나 경로를 직접 입력 → **항목 추가**로
사용자/그룹과 rwx를 지정합니다. 폴더라면 **기본(default) ACL**을 함께 걸어 두면 이후
새로 만들어지는 파일·폴더가 같은 권한을 상속하고, **재귀 적용**을 켜면 기존 하위 항목에도
한 번에 적용됩니다.

> ACL은 파일시스템이 지원해야 동작합니다(ext4·XFS·btrfs는 기본 지원). 지원하지 않거나
> `acl` 패키지가 없으면 화면 상단에 그 이유가 표시됩니다.
...
### 인벤토리 내보내기 / 가져오기
`Disk Inventory` 우측 상단 **내보내기**로 전체 인벤토리를 저장할 수 있습니다.
- **JSON** — 디스크 식별/상태 스냅샷 + 위치 메타데이터(백업·서버 이전용). 이 파일을 **가져오기**로 올리면 위치 라벨/베이/메모가 stableId 기준으로 복원됩니다(식별·건강·ZFS 정보는 항상 실시간 감지라 가져오지 않음).
- **CSV** — 모델·serial·WWN·용량·SMART·ZFS·위치를 표로 (스프레드시트/기록용).
...
- 기본(Caddy) 설치라면 자동 라우팅됩니다. `--no-caddy`로 자체 프록시를 쓴다면
  `/api/terminal/ws → 127.0.0.1:3001` WebSocket 라우팅이 있는지 확인하세요([2번 항목](#443-포트가-이미-쓰이는-경우)).
...
**로그인이 안 돼요** — ① 서버 OS 계정/비밀번호가 맞는지, 잠긴 상태(연속 실패)가 아닌지
확인(`journalctl -u nimbo`에 시도가 남습니다). ② **첫 로그인 때 등록된 IP 대역(/24)에서만**
접속됩니다 — 다른 네트워크면 비밀번호가 맞아도 거부됩니다([3번](#3-첫-로그인과-초기-설정)).
③ OS 비밀번호 검증에는 `python3`와 시스템 `libcrypt`가 필요합니다(설치 스크립트가 자동
설치·검증). 잠겼다면 `/etc/nimbo/users.json`의 `allowedCidrs`를 비우고
...
서비스 중지·비활성화, systemd 유닛·fail2ban jail·앱 번들·터미널 사이드카·sudo 규칙을
지웁니다. 설정(`/etc/nimbo`)·서비스 계정(`nimbo`)·Caddy 설정은 보존합니다.

설정·인증서·서비스 계정·Caddyfile 까지 **완전히** 삭제하려면:

```bash
sudo nimbo uninstall --purge
```

> `nimbo` 관리 CLI로 상태·로그·업데이트도 할 수 있습니다:
> `nimbo status` · `nimbo logs [app|term|caddy]` · `nimbo restart` · `nimbo update` ·
> `nimbo url` · `nimbo uninstall [--purge]` · `nimbo help`.

<details>
<summary>수동 제거 (nimbo CLI를 못 쓸 때)</summary>
