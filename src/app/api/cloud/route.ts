...
import { getCloudOverview, runCloudAction, type CloudAction } from "@/lib/system/rclone";
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as CloudAction;
    if (!body.kind) {
      return NextResponse.json({ ok: false, error: "action kind required" }, { status: 400 });
    }
    const result = await runCloudAction(body);
